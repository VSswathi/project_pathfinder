...
public class WaterfallCalc {
	...
	public WaterfallTableDto calculateValue( TotalSavingsModel2BDto modelb, TotalSavingsModel2ADto modela) {
		...
		obj.setYear1_with_partner(Math.abs(modelb.getModel2bcalculation().get(0).getTotals_client_spend_with_partner()-modelb.getTotals_baseyear()));
		obj.setYear1_without_partner(modelb.getModel2bcalculation().get(0).getTotals_client_spend_without_partner());
		obj.setYear2_with_partner(Math.abs(modelb.getModel2bcalculation().get(1).getTotals_client_spend_with_partner()-modelb.getModel2bcalculation().get(0).getTotals_client_spend_with_partner()));
		...
		obj.setYear3_with_partner(Math.abs(modelb.getModel2bcalculation().get(2).getTotals_client_spend_with_partner()-modelb.getModel2bcalculation().get(1).getTotals_client_spend_with_partner()));
