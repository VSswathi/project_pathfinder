...
// 14-pathfinder_run_it_outsource_fte
	...
	public RunItOutsourceFteDto runfteCalc( InputTablesDto inp,ItPersonnelDto itp,ItPersonalCostDto ipc) {
       ...
       obj.setNet_fte_scope_baseyear(obj.getClient_exist_baseyear()+obj.getPartner_exist_baseyear());   
        ...
        int i=ipc.getYearBseCalculations().size();
		for(int k=0;k<i;k++) {
        	RunItOutsourceFteYear  a = new RunItOutsourceFteYear();
            a.setYear(k+1);
            double take_over_plan_perc= ipc.getYearBseCalculations().get(k).getTakeOverPlan()/100;
                ...
                a.setPartner_exit_load((int) ipc.getYearBseCalculations().get(k).getNetPartnerFte());
...
// 15-pathfinder_run_it_outsource_costs
   ...
   obj.setTotal_spend_baseyear(obj.getClient_spend_baseyear()+obj.getPartner_bill_baseyear());
	...
	for(int k=0;k<i;k++) {
       ...
       double annual_inflation = inp.getAnnual_inflation_perc()/100;
       	...
       	a.setClient_spend((int) ( (fte.getRunfteCalc().get(k).getClient_exit_load()*itp.getAvr_ctc_per_fte())+(annual_inflation*(fte.getRunfteCalc().get(k).getClient_exit_load()*itp.getAvr_ctc_per_fte()))));
       	a.setPartner_bill((int) ((fte.getRunfteCalc().get(k).getPartner_exit_load()*ipc.getPartnerCtcOnsite())+(annual_inflation*((fte.getRunfteCalc().get(k).getPartner_exit_load()*ipc.getPartnerCtcOnsite())))));            
   ...
   obj.setNet_run_rate((int) Math.abs(initial_total_spend-obj.getTotal_spend_baseyear()));
...
// 16-pathfinder_itpersonnel_fit_model
	...
	long total_spend_original=0;
	 ...
	 int i=ipc.getYearBseCalculations().size();
	   ...
	   a.setClient_year(fte.getRunfteCalc().get(k).getClient_exit_load());
	   a.setClient_fte_year(cost.getRunCostsCalc().get(k).getClient_spend());
	   
	   a.setFte_onsite((int) ipc.getYearBseCalculations().get(k).getInFteOnsite());
	   a.setFte_offshore((int) ipc.getYearBseCalculations().get(k).getInFteOffshore());
...
// 17-pathfinder_asset_outsource_onsite
	...
	for(int k=0;k<i;k++) {
	   ...
	   a.setTotal_it_spend(cost.getRunCostsCalc().get(k).getTotal_spend());
	   a.setCot_partner_share((long) (itspend_percentage*cot.getYearBaseCostCalculations().get(k).getPartner_share_value()));
	   a.setTotal_savings_yoy((long) (a.getCot_partner_share()+(irp.getRunpersonnelcalculation().get(k).getYearlyRunRate()-a.getTotal_it_spend())));
	...
	public TotalOutsourcingFitshoreDto outsourceFitCalc(ItPersonnelDto itp, CostofTransformationDto cot, ItPersonnelFitModelDto ipf, ItSpendOnRunPersonnelDto run) {
		...
		for(int k=0;k<i;k++) {
	       ...
	       double it_spend_on_personnel_perc = itp.getIt_spend_on_personal_perc()/100;
	        ...
	        a.setOnsite_offshore_model(ipf.getFitcalc().get(k).getTotal_spend_for_original());
	        a.setCot_partner_share((long) (it_spend_on_personnel_perc*cot.getYearBaseCostCalculations().get(k).getPartner_share_value()));
	        a.setTotoal_outsource_fit(a.getOnsite_offshore_model()-a.getCot_partner_share());
	        
	        a.setTotal_savings_yoy((long) (a.getCot_partner_share()+(run.getRunpersonnelcalculation().get(k).getYearlyRunRate()-a.getOnsite_offshore_model())));
